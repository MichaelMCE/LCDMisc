...
EXPORT void CALLBACK Update(LcdInfo *info, BitmapInfo *bmp) {
	info = info;
...
EXPORT void CALLBACK Destroy(LcdInfo *info) {
...
int registered = 0;
EXPORT int CALLBACK lcdInit(LcdCallbacks *callbacks) {
	callback = callbacks;
	WNDCLASS wndclass = {
		0,
		WndProc,
...
EXPORT void CALLBACK lcdUninit() {
	callback = 0;
	for (int i=0; i<2; i++) {
...
EXPORT LcdInfo * CALLBACK lcdEnum() {
	for (int i=0; i<2; i++) {
