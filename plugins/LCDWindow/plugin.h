...
typedef int (CALLBACK * lcdStart)(struct LcdInfo *info);
...
typedef void (CALLBACK * lcdStop)(struct LcdInfo *info);

// Function specified by LCDInfo structure.
// Info contains what lcdEnum returned.
// Final function called on info.  Not required.
typedef void (CALLBACK * lcdDestroy)(struct LcdInfo *info);
...
typedef void (CALLBACK * lcdUpdate)(struct LcdInfo *info, BitmapInfo *bmp);
