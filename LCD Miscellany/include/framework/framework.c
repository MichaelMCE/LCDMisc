...
struct EventHandler extends ObjectList {
	...
	function EventHandler() {
		...
		%tbtnOverlays = 0;
		%btnOverlays = list();
		%btnOverlays[%tbtnOverlays++] = touchArea(2, 102, 72, 172, "G15ButtonDown", G15_LEFT);
		%btnOverlays[%tbtnOverlays++] = touchArea($_W-72, 102, $_W-2, 172, "G15ButtonDown", G15_RIGHT);
		%btnOverlays[%tbtnOverlays++] = touchArea(($_W/2)-(70/2), 2, ($_W/2)+(70/2), 70, "G15ButtonDown", G15_UP);
		%btnOverlays[%tbtnOverlays++] = touchArea(($_W/2)-(70/2), $_H-70, ($_W/2)+(70/2), $_H-2, "G15ButtonDown", G15_DOWN);
		
		%btnOverlays[%tbtnOverlays++] = touchArea(($_W/2)-(82/2), 102, ($_W/2)+(82/2), 172, "G15ButtonDown", G15_OK);
		%btnOverlays[%tbtnOverlays++] = touchArea(2, $_H-70, 82, $_H-2, "G15ButtonDown", G15_CANCEL);
		%btnOverlays[%tbtnOverlays++] = touchArea($_W-82, $_H-70, $_W-2, $_H-2, "G15ButtonDown", G15_MENU);
		%btnOverlays[%tbtnOverlays++] = touchArea(0, 0, 69, 69, "G15ButtonDown", SHOW_CLOCKA);
		%btnOverlays[%tbtnOverlays++] = touchArea($_W-70, 0, $_W, 69, "G15ButtonDown", SHOW_CLOCKD);
...
struct MenuHandler {
	...
	function MenuHandler($Handler) {
		...
		%AddView(@$);
		toolbarVisible = 0;
		%toolbarIconBack = LoadImage32("images/back.png");

		%buttons = Icons();		
		%buttons.addIcon(2, 6,  104,"images\left.png", "G15ButtonDown", G15_LEFT);
		%buttons.addIcon(2, 412,104,"images\right.png","G15ButtonDown", G15_RIGHT);
		//%buttons.addIcon(2, 208, 1, "images\up.png",  "G15ButtonDown", G15_UP);
		//%buttons.addIcon(2, 208,168,"images\down.png","G15ButtonDown", G15_DOWN);
		//%buttons.addIcon(2, 208, 104,"images\ok.png", "G15ButtonDown", G15_OK);
		//%buttons.addIcon(2, 1  ,206,"images\cancel.png", "G15ButtonDown", G15_CANCEL);
	...
	function sbuiDKDown ($event, $param, $key) {
		...
		if ($key == SBUI_DK_1 || $key == SBUI_DK_5){
	...
	function Draw ($event, $param, $name, $res) {
		...
		/*}else if (!drawClock ){
			...
			clock.DrawForeground(1);		// analogue
