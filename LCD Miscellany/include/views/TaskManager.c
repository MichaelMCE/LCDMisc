...
struct TaskManager extends View {
	...
	function TaskManager($noKeySteal, $_smallFont, $_bigFont) {
		...
		%selectedHwndIndex = -1;
		%keySteal = !$noKeySteal;
		%toolbarImage = LoadImage32("Images\process1.png");
		%toolbarImageL = LoadImage32("Images\process2.png");
		%background = LoadImage32("Images\taskmanBackground.png");
	...
	function Draw ($event, $param, $name, $res) {
		...
		if (%hasFocus) {
			$bgColor = RGBA(140,140,140,120);
		}else if ($bpp > 1){
			$bgColor = RGBA(80,80,80,120);
		}else{ $inv = -1;
	}
