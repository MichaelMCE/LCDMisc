...
struct MediaView extends View {
	...
	function MediaView () {
		...
		for ($i=0; $i<size($); $i++) {
			if (!IsNull($[$i])){
				...
				%mpIcons.addIcon(1, 410, 46, $[$i].bigMediaImage, "MediaControl", $alist);
			}
		}
		
		%toolbarImage = LoadImage32("Images\mediaplayer1.png");
		%toolbarImageL = LoadImage32("Images\mediaplayer2.png");
		%background = LoadImage32("Images\mediaplayerBackground.png");
		
		%bigFont = Font("Lucida Sans Unicode", 20,0,0,0,CLEARTYPE_QUALITY);
		%bigTitleFont = Font("Lucida Sans Unicode", 26,0,0,0,CLEARTYPE_QUALITY);
		...
		%icons = Icons();
		%AddButton(90,156,  "Images\media\prev.png", "MediaControl", VK_MEDIA_PREV_TRACK);
		%AddButton(170,156, "Images\media\play.png", "MediaControl", VK_MEDIA_PLAY_PAUSE);
		%AddButton(250,156, "Images\media\next.png", "MediaControl", VK_MEDIA_NEXT_TRACK);
		%AddButton(330,156, "Images\media\stop.png", "MediaControl", VK_MEDIA_STOP);
		%AddButton(210,88,  "Images\media\sound.png","MediaControl", VK_VOLUME_MUTE);
	...
	function doSVMEMUpdate (){
		if (!IsNull(%mapping)){
			...
			if (!IsNull(%mapping)){
				%view = %mapping.MapViewOfFile(0, 64+(640*480*4));
				if (%updateTimerId)
					ModifyFastTimer(%updateTimerId, 50); // we only want 20fps
			}
		}
		%SVMemFrameCt = 0;
	}
	...
	function Show() {
		if (!%scrollTimer) {
			...
			%bigTitleScroller = ManualScrollingText(,480,1,1,28);
			%bigArtistScroller = ManualScrollingText(,480,1,1,22);
			%bump = 0;
	...
	function DrawG19 () {
		...
		if (size($p.specHist)) {
			...
			DrawSpectrum2(%specCache, $p.specHist, 0, 110, 479, 271);
		}else if (size($p.spectrum)) {
			DrawSpectrum($p.spectrum, 0, 110, 479, 271);
		}else if (size($p.waveform)) {
			DrawWave($p.waveform, 0, 110, 479, 271);
		}

		if (%hasFocus == 1) 
			ColorRect(0, 46, 479, 64, RGBA(80,80,80,160));
		else	
			ColorRect(0, 46, 479, 64, RGBA(40,40,40,120));
		...
		if ($p.state >= 0) {
			if (!%selected) {
				ColorRect(0, 0, 479, 44, RGBA(80,80,80,160));
				SetDrawColor(colorHighlightText);
			}
			if (size($p.artist)) {
			...
			if ($p.state != 0) {
				...
				DisplayTextRight(FormatDuration($p.position), 56, 44);
				%drawBargraph(60, 49, 280, 12, (100.0/$p.duration)*$p.position);
				if ($p.duration <= 0 && $p.position) {
					$duration = "N/A";
				}else {
					$duration = FormatDuration($p.duration);
				}
				DisplayText($duration, 345, 44);
			}
		
			ColorRect(1, 69, 390, 69+15, RGBA(20,20,20,80));
				
			if ($p.tracks)
				DisplayTextRight($p.track +s "/" +s $p.tracks, 388, 66);
			SetDrawColor(colorText);

			$v = "Volume: " +s $p.volume +s "%";
			DisplayText($v, 2, 66);
			if ($p.muted) {
				DrawLine(2, 74, TextSize($v)[0], 74);
			}
			...
			if (%selected == 1) {
				$s = TextSize($p.mode)[0];
				ColorRect(260 - $s/2,68, 261 + ($s+1)/2, 84, RGBA(80,80,80,160));
				SetDrawColor(colorHighlightText);
			}
			DisplayTextCentered($p.mode, 260,66);
		}else {
			%selected = -1;
		}
