...
struct LineEditor {
	...
	function DrawBox($_text) {
		...
		$totalH = $fh + $h + 23;
		$top = 21-$totalH/2;
		DrawRect(0, $top, $_W, $top);
		$bottom = $totalH+$top-1;
		ClearRect(0, $top+1, $_W, $bottom-1);
		DrawRect(0, $bottom, $_W, $bottom);
