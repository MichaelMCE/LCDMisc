...
struct GenericImage {
	...
	unsigned char GetPixel(Color4 *c, int x, int y) const;
	// Assumes pixel is within image.
	unsigned char GetPixelBilinear(Color4 *c, float x, float y) const;
};
 //<unsigned char>
template <class T>
inline unsigned char GenericImage<T>::GetPixel(Color4 *c, int x, int y) const {
...
template <class T>
inline unsigned char GenericImage<T>::GetPixelBilinear(Color4 *c, float x, float y) const {
	float x1, x2, y1, y2;
	int wx1, wx2, wy1, wy2;
...
int Blur (ScriptValue &srcv, ScriptValue &dst, int radius, int blueType);
...
int CreateImage32 (ScriptValue &srcv, ScriptValue &dst, int width, int height);
