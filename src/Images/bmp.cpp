...
int SaveBMP (FILE *file, const int width, const int height, const int spp, unsigned char *pixels)
{
	if (!file) return 0;
	unsigned int i, j;
	tagBITMAPINFO info;
	...
	*(int*)z = 0;
	...
	long lineSize = (3*width+3)&~3;

	unsigned char *line = (unsigned char*)calloc(1, lineSize+8);
	if (!line) return 0;
	const unsigned long memWidth = (width*spp+3)&~3;

	if (spp >= 3) {
		for (i=height; i--;) {
			int p = 0;
			for (j=0; j<width; j++) {
				line[p++] = pixels[i*memWidth + j*spp]; // b
				line[p++] = pixels[i*memWidth + j*spp+1]; // g
				line[p++] = pixels[i*memWidth + j*spp+2];   // r
		...
		for (i=height; i--;) {
			...
			for (j=0; j<width; j++) {
				line[p] =
					line[p+1] =
					line[p+2] = pixels[i*memWidth + j*spp];
				p+=3;
				//alpha |= 255 - in->pixels[i*in->width + j].a;
			}
			fwrite(line, 1, lineSize, file);
...
{
	return SaveBMP(file, in->width, in->height, in->spp, in->pixels);
}
