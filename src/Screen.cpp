...
Screen::Screen (int width, int height, int bpp)
{
	...
	activeTextColor.val = 0xFF00FF00;
	activeBgColor.val = 0xFFFFFF00;

	activeBgMode = OPAQUE;

	bgColor.val = 0xFF00FFFF;
	drawColor.val = 0xFF00FF00;
	this->width = width;
	this->height = height;
...
void Screen::SetFont() {
	if (!hPrevFont) {
		...
		prevTextColor = SetTextColor(hDC, RGBA(255, 255, 255, 255));
		prevBkColor = SetBkColor(hDC, RGBA(255, 0, 0, 255));
...
int Screen::DisplayTextRight(int x, int y, unsigned char *text, int len, int highlight, int mode)
{
	...
	while (pos < len) {
		...
		while (temp > start && (text[temp-1] == ' ' || text[temp-1] == '\t'))
			temp--;
...
static inline void FlipBR (Color4 *c)
{
...
void Screen::DisplayText (int x, int y, unsigned char *text, int len, int highlight, int mode, RECT *clip, int flags)
{
	...
	if (f->hFont){
		...
		if (useTextColor.val != activeTextColor.val){
			activeTextColor.val = useTextColor.val;
			SetTextColor(hDC, activeTextColor.val);
		}

		if (useBgMode != TRANSPARENT && useBgColor.val != activeBgColor.val)
			SetBkColor(hDC, activeBgColor.val=useBgColor.val);

		//printf("%X %X '%s'\n", activeTextColor.val, useTextColor.val, text);
...
void Screen::DisplayImage (int dstx, int dsty, int srcx, int srcy, int width, int height, const GenericImage<unsigned char> *img)
{
	...
	//printf("DisplayImage 2 %i %i, %i %i, %i\n",dx, dy, width, height, img->spp);
...
int Screen::SaveSnapshot (const char *path8)
{
	int ret = 0;
	int len = strlen(path8);

	wchar_t *pathw = UTF8toUTF16Alloc(path8, &len);
	if (pathw){
		FILE *f = _wfopen(pathw, L"wb");
		if (f){
			ret = SaveBMP(f, width, height, bpp/8, (unsigned char*)image);
			//fclose(f);
		}
		free(pathw);
	}
