	...
	switch (string[0]) {
		...
		case '\'':
			tokens[numTokens].type = TOKEN_STRING;
			{
				...
				const int len = string - start;
				if (string >= end || !(tokens[numTokens].val = (unsigned char*)malloc(sizeof(unsigned char)*(len+1)))){
					tokens[numTokens++].type = TOKEN_ERROR;
					LexError(line+1, lineStart, string-1, "String not closed");
					return 1;
				}

				memcpy(tokens[numTokens].val, start, len);
				tokens[numTokens].val[len] = 0;
