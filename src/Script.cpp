...
int RegisterProcs ()
{
	...
	if (!VM.Globals.Add(VM.StringTable.table[1].stringVal))
		return 0;
	...
	static const unsigned char lists[][9] = {
		...
		{C_obj4ints,SCRIPT_OBJECT, 1, SCRIPT_INT, 4, 0, 0},
		{C_objint,SCRIPT_OBJECT, 1, SCRIPT_INT, 1, 0, 0},
		...
		{C_2intsstring,SCRIPT_INT,2, SCRIPT_STRING,1,0, 0},
	...
	for (int i=0; i<sizeof(lists)/sizeof(lists[0]); i++){
		if (!argLists.AddList(lists[i][0], lists[i]+1))
			return 0;
	}
	/*
...
int LoadScript (int i, int import, const int writeLog)
{
	// Already tried.
	if (files[i].loaded != -1)
		return 3;
	...
	int res = 0;
	FileLineReader *reader = OpenLineReader(files[i].name, 0, WHOLE_FILE,0, 1000000);
	if (reader){
		unsigned char *data;
