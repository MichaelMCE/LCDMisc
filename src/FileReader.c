...
// Needed for 64-bit compatibility.  No clue why.  Switching this over to a C++
// file might help, but... why?  Easier to do this than remove file, rename, and
// add it back, anyways.
//#define swprintf FUCK_A_GOAT
...
int readerMiniseek(FileReader *in, int pos, int mode) {
	if (mode != SEEK_SET) return 0;
	if (pos <0) return 0;
	if (in->buffer.used >= pos) in->buffer.read = pos;
	return 1;
}

FileReader *wreaderOpen(wchar_t *name)
{
	FileReader *fileReader = readerOpenSub(_wfopen(name, L"rb"), 0);
	if (!fileReader) {
		wchar_t *temp = (wchar_t*) malloc(sizeof(wchar_t)*(wcslen(name)+10));
		swprintf(temp, L"%s.Z", name);
		if (!(fileReader = readerOpenSub(_wfopen(temp, L"rb"), 1))) {
			swprintf(temp, L"%s.gz", name);
			fileReader = readerOpenSub(_wfopen(temp, L"rb"), 2);
		}
		free(temp);
	}
