...
wchar_t *GetFile(wchar_t *name)
{
	...
	if (name[1] == L':' && (name[2] == '\\' || name[2] == '/'))
		return wcsdup(name);

	int temp = GetModuleFileNameW(0, s, MAX_PATH+2);
	if (temp >= MAX_PATH+2 || temp == 0)
		return 0;
...
FileLineReader* OpenLineReaderB (wchar_t *fileName, int ignoreEmptyLines, int type, const unsigned char *comments, int maxLineLen)
{
	//MessageBoxW(0, L"e", L" ", MB_OK);
	if (!fileName) return 0;
	//MessageBoxW(0, L"f", L" ", MB_OK);
	FileReader *f = wreaderOpen(fileName);
	//MessageBoxW(0, L"g", L" ", MB_OK);
	if (!f) return 0;
...
static SHChangeNotifyEntry shcne;
static unsigned int shellNotifyId = 0;
static HDEVNOTIFY hDeviceNotify[4];

static inline void RegisterDeviceInterfaceToHwnd (GUID InterfaceClassGuid, HWND hWnd, HDEVNOTIFY *hDeviceNotify)
{
...
static inline UINT shChangeNotifyRegister (HWND hWnd, DWORD dwFlags, LONG wEventMask, UINT uMsg, DWORD cItems,SHChangeNotifyEntry *lpItems)
{
	return SHChangeNotifyRegister(hWnd, dwFlags, wEventMask, uMsg, cItems, lpItems);
}

static inline WINBOOL shChangeNotifyDeregister (unsigned long ulID)
{
	return SHChangeNotifyDeregister(ulID);
}

HANDLE shChangeNotification_Lock (HANDLE hChangeNotification, DWORD dwProcessId, LPITEMIDLIST **pppidl, LONG *plEvent)
{
	return SHChangeNotification_Lock(hChangeNotification, dwProcessId, pppidl, plEvent);
}

WINBOOL shChangeNotification_Unlock (HANDLE hLock)
{
	//if (!pSHChangeNotification_Unlock) return 0;

	return SHChangeNotification_Unlock(hLock);
}
...
unsigned int RegisterShellNotify (HWND hWnd, SHChangeNotifyEntry *shcne, unsigned int *notifyId)
{
	shcne->pidl = NULL;
   	...
   	long fEvents = SHCNE_DRIVEADD | SHCNE_DRIVEREMOVED | SHCNE_MEDIAREMOVED | SHCNE_MEDIAINSERTED;
   		//fEvents |= SHCNE_DISKEVENTS | SHCNE_ALLEVENTS;

	*notifyId = shChangeNotifyRegister(hWnd, SHCNF_ACCEPT_INTERRUPTS|SHCNF_ACCEPT_NON_INTERRUPTS|SHCNF_NO_PROXY, fEvents, WMU_MEDIA_CHANGE, 1, shcne);
   	return *notifyId;
}

void InitDriveNotification (HWND hWnd)
{
	RegisterShellNotify(hWnd, &shcne, &shellNotifyId);
}


void UninitDriveNotification ()
{
	shChangeNotifyDeregister(shellNotifyId);
}

void InitDeviceNotification (HWND hWnd)
{
	RegisterDeviceInterfaceToHwnd(GUID_DEVINTERFACE_USB_HUB, hWnd, &hDeviceNotify[0]);
	RegisterDeviceInterfaceToHwnd(GUID_DEVINTERFACE_USB_DEVICE, hWnd, &hDeviceNotify[1]);
	RegisterDeviceInterfaceToHwnd(GUID_DEVINTERFACE_USB_HID, hWnd, &hDeviceNotify[2]);
}
...
void UninitDeviceNotification ()
{
	...
	memset(hDeviceNotify, 0, sizeof(hDeviceNotify));
}


int IsUsbDevice (const char drive)
{

	wchar_t buffer[128];
	_snwprintf(buffer, 128, L"\\\\.\\%c:", (wchar_t)drive);
...
float __cdecl my_logf (float val)
{
	return logf(val);
}

float __cdecl my_sinf (float val)
{
	return sinf(val);
}

float __cdecl my_cosf (float val)
{
	return cosf(val);
}

float __cdecl my_log10f (float val)
{
	return log10f(val);
}

float __cdecl my_acosf (float val)
{
	return acosf(val);
}

float __cdecl my_asinf (float val)
{
	return asinf(val);
}

float __cdecl my_tanf (float val)
{
	return tanf(val);
}

float __cdecl my_atanf (float val)
{
	return atanf(val);
}

float __cdecl my_sqrtf (float val)
{
	return sqrtf(val);
}

float __cdecl my_expf (float val)
{
	return expf(val);
}

float __cdecl my_powf (float val1, float val2)
{
