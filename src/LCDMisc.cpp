...
#define EXEC_MESSAGE ((int64_t)0x6543263461789675)
...
bool gOnTray = 0;
bool noLCD = 0;
static HANDLE volumeChangeEvent;
...
static int FlushImage (const int drawn)
{

	static uint64_t lastDrawn = 0;
	static uint64_t lastFlushed = 0;
	static uint64_t lastDeviceEnum = 0;
	...
	for (int i = 0; i < numDevices; i++){
		if (devices[i]->sendImage){
			//printf("FlushImage %i, %i %i\n", i, devices[i]->sendImage, numDevices);
	...
	uint64_t t = GetTickCountNoOverflow();
...
static inline void Draw ()
{
	...
	if (redrawn || (uint64_t)(tick - lastDraw) >= 5){
		if (!FlushImage(1))
			lastDraw = tick - 10;
		else
			lastDraw = tick;
...
static inline void onDeviceArrive (const unsigned int type, const int vid, const int pid)
{
	//printf("Device Arrive: vid %.4X, pid %.4X\n", vid, pid);

	TriggerEvent(deviceChangeEvent, DEVICE_ARRIVE, vid, pid);	
}

static inline void onDeviceDepart (const unsigned int type, const int vid, const int pid)
{
	//printf("Device Depart: vid %.4X, pid %.4X\n", vid, pid);

	TriggerEvent(deviceChangeEvent, DEVICE_DEPART, vid, pid);	
}

static inline int onDeviceChangeEvent (const WPARAM wParam, const LPARAM lParam)
{
	DEV_BROADCAST_HDR *hdr = (DEV_BROADCAST_HDR*)lParam;
	/*if (!hdr)
		printf("WM_DEVICECHANGE %i\n", wParam);
	else
	...
	switch (wParam){
	  case DBT_DEVICEARRIVAL:
	  	...
	  	/*if (hdr->dbch_devicetype == DBT_DEVTYP_VOLUME){
	  		...
	  		if (guidToId(dbi->dbcc_name, &vid, &pid)){
	  			onDeviceArrive(((int*)&dbi->dbcc_classguid)[0], vid, pid);
	  			return 0;
	  		}
	  	}
	  	break;
	  ...
	  case DBT_DEVICEREMOVECOMPLETE:
	  	...
	  	/*if (hdr->dbch_devicetype == DBT_DEVTYP_VOLUME){
	  		...
	  		int vid, pid;
	  		if (guidToId(dbi->dbcc_name, &vid, &pid)){
	  			onDeviceDepart(((int*)&dbi->dbcc_classguid)[0], vid, pid);
	  			return 0;
	  		}
	  	}
	  	break;
...
static inline int onMediaChangeEvent (const WPARAM wParam, const LPARAM lParam)
{
	...
	if (hLock && ppidl){
		char drivePath[2047+1];

		if (!SHGetPathFromIDListA(ppidl[0], drivePath))
			return 0;
		...
		switch (lEvent){
	  	...
	  	case SHCNE_DRIVEREMOVED:
	  		msgType = DRIVE_REMOVED;
			break;
	  	case SHCNE_MEDIAINSERTED:
	  		msgType = DRIVE_MEDIA_INSERTED;
			break;
	  	case SHCNE_MEDIAREMOVED:
			msgType = DRIVE_MEDIA_REMOVED;
			break;
		default:
			return;
		}
		...
		if (msgType){
	  		int64_t isUSB = IsUsbDevice(drivePath[0]);
	  		//printf("is Drive %i, msgType:%i, #%s#\n", isUSB, msgType, drivePath);
	  		TriggerEvent(driveChangeEvent, msgType, drivePath[0], isUSB);
	  		return 1;
		}
	}
...
static LRESULT CALLBACK WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
			...
			if (wParam == 0) {
				...
				if (EventExists(foregroundWindowChangeEvent)) {
					...
					if (foregroundWindow != lastForegroundWindow) {
						...
						TriggerEvent(foregroundWindowChangeEvent, (int64_t)foregroundWindow, (int64_t)temp);
					}
				}
				int running = 0;
				...
				for (int w=0; w<numDevices; w++) {
					...
					// Don't think it can happen, but just in case...
					sv[1].AddRef();
					int i = 1;
	...
	switch (uMsg) {
		...
		case WMU_MEDIA_CHANGE:
			onMediaChangeEvent(wParam, lParam);
			return 0;
...
/*		case WMU_PING_SOCKET:
		...
		case WM_CREATE:
			...
			volumeChangeEvent = CreateEvent(NULL, 0, 0, L"LCDMiscVolumeChange");
			InitDriveNotification(hWnd);
			InitDeviceNotification(hWnd);

			break;
		case WM_DESTROY:
			UninitDriveNotification();
			UninitDeviceNotification();
		...
		case WM_DRAWCLIPBOARD:
			if (nextClipboardListener)
				SendMessage(nextClipboardListener, WM_DRAWCLIPBOARD, wParam, lParam);
...
int WINAPI WinMain(	HINSTANCE hInst,
...

	{
		...
		if (!InitEvents() || !(VM.globals = (ScriptValue*)malloc(sizeof(ScriptValue) * VM.Globals.numValues))) {
		...
		for (int i = 0; i < VM.Globals.numValues; i++)
	...
	if (SetTimer(ghWnd, 0, 1000, 0)) {
...

		{
			int i = 0;
			while (1) {
				unsigned char *temp = config.GetConfigString((unsigned char*)"Script", (unsigned char*)"InitProc", (unsigned char*)0, 0, i);
		...
		if (errors || warnings){
			...
			if (errors){
				...
				if (errors > 1)
					sprintf(errorString, "%i errors\n", errors);
				else
					strcpy(errorString, "1 error\n");
				flag = MB_ICONEXCLAMATION;
			}

			if (warnings){
				if (warnings > 1)
					sprintf(strchr(errorString, '\0'), "%i warnings\n", warnings);
				else
					strcpy(strchr(errorString, '\0'), "1 warning\n");
			}
