...
volatile inline FileLineReader* OpenLineReader(wchar_t *fileName, int ignoreEmptyLines, int type, const char *commentChars, int maxLineLen = 1000000)
{
	const unsigned char *uc = (const unsigned char*)commentChars;
	return OpenLineReaderB(fileName, ignoreEmptyLines, type, (const unsigned char*)uc, maxLineLen);
}
...
// appends add to end of base, if add is not an absolute path.
