...
{
	if (args[1].intVal < 0 || args[0].stringVal->len <= args[1].intVal)
		return;
	
	unsigned int pos = args[1].i32;
	int len = args[0].stringVal->len - (int)pos;
	unsigned int c = (unsigned int)NextUnicodeChar(args[0].stringVal->value+pos, &len);
	
	//printf("%i, #%s#\n", args[0].stringVal->len, args[0].stringVal->value+pos);
	//printf("pos %i, %i\n", pos, c);
