...
void SaveSnapshot (ScriptValue &s, ScriptValue *args)
{
	//printf("ScriptDrawing SaveSnapshot %s, %i\n", args[0].stringVal->value, numDevices);
	//int ret = 0;
	//for (int i = 1; i < numDevices; i++)
	//	ret = devices[i]->SaveSnapshot(args[0].stringVal->value);
	//CreateIntValue(s, ret);

	int ret = 0;
	if (numDevices)
		ret = devices[numDevices-1]->SaveSnapshot(args[0].stringVal->value);
	CreateIntValue(s, ret);
}
