...
{
	CreateIntValue(s, (__int64)GetForegroundWindow());
}

void SetForegroundWindow (ScriptValue &s, ScriptValue *args)
{
	SetForegroundWindow(args[0].intVal);
	SetActiveWindow(args[0].intVal);
	CreateIntValue(s, SetFocus(args[0].intVal));
}

void ShowWindow (ScriptValue &s, ScriptValue *args)
{
	CreateIntValue(s, ShowWindowAsync((HWND)args[0].intVal, args[1].intVal));
}
