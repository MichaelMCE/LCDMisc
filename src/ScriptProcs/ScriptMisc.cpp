...
static inline BOOL GetFocussedProcess (wchar_t *procfile, DWORD procfilelen)
{
	*procfile = 0;
	...
	if (!QueryFullProcessImageNameW(hproc, 0, procfile, &procfilelen)){
		...
		*procfile = 0;
		if (!GetModuleFileNameExW(hproc, NULL, procfile, MAX_PATH))
			*procfile = 0;
	}
...
void GetFocussedProcessPath (ScriptValue &s, ScriptValue *args)
{
	...
	if (GetFocussedProcess(procfile, &procfilelen)){
		char *name = UTF16toUTF8Alloc(procfile);
		if (name){
...
static inline BOOL IsCurrentUserLocalAdministrator ()
{
...
{
	CreateIntValue(s, IsCurrentUserLocalAdministrator());
}
...
void GetDesktopInfo (ScriptValue &s, ScriptValue *args)
{
	...
	if (CreateListValue(s, 4)){
  		CreateIntValue(s.listVal->vals[0], (int)devmode.dmPelsWidth);
  		CreateIntValue(s.listVal->vals[1], (int)devmode.dmPelsHeight);
  		CreateIntValue(s.listVal->vals[2], (int)devmode.dmBitsPerPel);
  		CreateIntValue(s.listVal->vals[3], (int)devmode.dmDisplayFrequency);
  		s.listVal->numVals = 4;
  		return;
	}
}
...
void Is64Bit (ScriptValue &s, ScriptValue *args)
{
	...
	if (!args[0].intVal){
		...
		//is64 = (sSysInfo.DUMMYUNIONNAME.DUMMYSTRUCTNAME.wProcessorArchitecture == PROCESSOR_ARCHITECTURE_AMD64);
	}else{
		is64 = (sizeof(intptr_t) == 8);
	}

	CreateIntValue(s, is64);
}
	...
	SYSTEM_POWER_STATUS ps;
  	...
  	if (GetSystemPowerStatus(&ps)){
  		if (CreateListValue(s, 5)){
  			CreateIntValue(s.listVal->vals[0], (int)ps.BatteryLifePercent);
  			CreateIntValue(s.listVal->vals[1], (int)ps.BatteryLifeTime);
  			CreateIntValue(s.listVal->vals[2], (int)ps.BatteryFullLifeTime);
  			CreateIntValue(s.listVal->vals[3], (int)ps.ACLineStatus);
  			CreateIntValue(s.listVal->vals[4], (int)ps.BatteryFlag);
  			s.listVal->numVals = 5;
  			return;
  		}
  	}
