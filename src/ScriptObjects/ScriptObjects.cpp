...
int RegisterObjectTypes() {
...
#if 1
	{
		...
		const ProcDescription create[] = {{"ComPort", ComPortOpen, C_string4ints}};
		const ProcDescription script[] = {{"Write", ComPortWrite, C_objint},
										  {"Read", ComPortRead, C_int},
										  {"Close", ComPortClose, C_noArgs}};
		const ProcDescription dest[]   = {{"ComPort::~ComPort", CleanupComPort, C_noArgs}};
		res &= CreateObjectType("ComPort", vals, sizeof(vals)/sizeof(vals[0]), create, sizeof(create)/sizeof(create[0]), script, sizeof(script)/sizeof(script[0]), dest);
		ComPortType = count++;
	}
#endif
...

	{
		...
		const ProcDescription create[] = {{"LoadImage32", LoadImage32, C_string},
										{"NewImage32", NewImage32, C_2ints},
										{"LoadMemoryImage32", LoadMemoryImage32, C_string},
		};
		const ProcDescription script[]={{"Size", Image32Size, C_noArgs},
										{"Copy", Image32Copy, C_noArgs},
										{"Clear", Image32Clear, C_int},
										{"Draw", Image32Draw, C_obj6ints},
										{"Zoom", Image32Zoom, C_double},
										{"Fade", Image32Fade, C_doubleint},
										{"Blur", Image32Blur, C_2ints},
										{"Flip", Image32Flip, C_int},
										{"Pixelize", Image32Pixelize, C_int},
...
/*	{
	...
	}
	{
		...
		const ProcDescription create[] = {{"IPAddr", IPAddrSV, C_stringint},
