...
unsigned char* App::GetVal(unsigned char *key, int type, int index)
{
	...
	int res;
	if (!(type & 2)) {
		if ((res = GetValStatic(key, strlen(key), index)) >= 0){
			//MessageBoxA(0, pairsStatic[res].value->value, key, MB_OK);
			return pairsStatic[res].value->value;
		}
	}
	if (index) return 0;

	if (!(type & 1)) {
		if ((res = GetValDynamic(key, strlen(key))) >= 0){
			//MessageBoxA(0, pairs[res].value->value, key, MB_OK);
			return pairs[res].value->value;
		}
	}
	return 0;
}
...
{
	//MessageBoxA(0, key, "get val static", MB_OK);
...
int App::GetValDynamic (unsigned char *key, int len)
{
	for (int i=0; i < numPairs; i++) {
		if (pairs[i].key->len == len && strncmp(key, pairs[i].key->value, len) == 0)
			return i;
	}
	return -1;
}
...
{
	for (int i=0; i<numApps; i++) {
		if (stricmp(apps[i].name, app)==0)
			return &apps[i];
	}

	if (!srealloc(apps, sizeof(App)*(numApps+1)))
		return 0;
...
void Config::Reload ()
{
	...
	for (int i = 1; i < 3; i++) {
		...
		if (GetConfFile(fileName, ext) && (in = OpenLineReaderB(fileName, 1, SINGLE_LINE, (const unsigned char*)"#%", 1000000))) {
...
unsigned char *Config::GetConfigString (unsigned char *appName, unsigned char*key, unsigned char *defaultVal, int type, int index)
{
	...
	//MessageBoxA(0, val, apps->name, MB_OK);
